...
function applyTheme(t, { animate = false } = {}) {
    ...
    localStorage.setItem("vocallyze-theme", t);
    if (animate) {
...
export default function ThemeProvider({ children }) {
    ...
    useEffect(() => {
        // One-time migration from the old storage key so returning visitors
        // keep their chosen theme instead of silently resetting to arctic.
        let stored = localStorage.getItem("vocallyze-theme");
        if (!stored) {
            const legacy = localStorage.getItem("admino-theme");
            if (legacy) {
                stored = legacy;
                localStorage.setItem("vocallyze-theme", legacy);
                localStorage.removeItem("admino-theme");
            }
        }
