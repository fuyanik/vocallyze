...
import { AGENTS, MONTHLY, QUALITY_BUBBLES, SCORE_DIST, WEAK_AGENT } from "../data";
    ...
    <Shell active="agents" height={CANVAS_H}>
      <div data-shot="hdr">
        <PageHeader
          ...
          subtitle="Every agent measured on the same rubric, across every single call"
          actions={
            <>
      ...
      <div data-shot="kpis" className="mb-3.5 grid grid-cols-4 gap-2.5">
        <StatCard label="Average score" value={<Counter to={86} play={kpi} />} unit="/100" delta={3} icon="target" tone="blue" />
        <StatCard label="Calls analysed" value={<Counter to={2551} play={kpi} />} delta={12} icon="phone" tone="teal" hint="100% coverage" />
        <StatCard label="First-contact resolution" value="79%" delta={6} icon="checkCircle" tone="green" />
        <StatCard label="Open findings" value={<Counter to={27} play={kpi} />} delta={-34} invertDelta icon="alert" tone="orange" />
      </div>
      ...
      <div className="mb-3.5 flex gap-3.5">
        <Card shot="trend" className="min-w-0 flex-1">
          <CardHeader
            ...
            right={<Badge tone="green">+14 points since February</Badge>}
          />
          <LineChart
            w={356}
            h={200}
            ...
            xLabels={MONTHLY.x}
            series={[
              { label: "Quality score", color: C.blue, data: MONTHLY.score },
              { label: "Resolution rate", color: C.green, data: MONTHLY.resolution },
            ]}
            dots
            play={trend}
          />
          <Legend
            className="mt-2"
            items={[
              { label: "Quality score", color: C.blue },
              { label: "Resolution rate", color: C.green },
            ]}
          />
        </Card>

        <Card shot="dist" className="w-[248px] shrink-0">
          <CardHeader title="Score distribution" subtitle="Agents per score band" />
          ...
          <div className="mt-3 pt-3" style={{ borderTop: `1px solid ${T.border}` }}>
            ...
            <p className="mt-1 text-[10px]" style={{ color: T.faint }}>
              Bubble size shows the number of findings
            </p>
          </div>
        </Card>
      </div>
        ...
        <div className="flex items-center justify-between px-5 py-4">
          <div>
            ...
            <p className="mt-0.5 text-[11px]" style={{ color: T.faint }}>
              Sorted by score · findings column counts evidence-backed violations
            </p>
          </div>
          <Badge tone="gray">8 active</Badge>
        </div>
        <div className="grid grid-cols-[1.7fr_1fr_60px_70px_64px_86px] items-center gap-2 px-5 pb-2">
          {["Agent", "Team", "Calls", "Findings", "Score", "7-week trend"].map((h) => (
            ...
            return (
              ...
              >
                ...
                <span className="flex items-center gap-1.5">
                  ...
                  {a.crit > 0 && <Badge tone="red">{a.crit} crit</Badge>}
                </span>
                <span
                ...
                <div className="flex items-center gap-3.5">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge tone="red">{WEAK_AGENT.crit} critical findings</Badge>
                    </div>
                    <p className="mt-0.5 text-[11.5px]" style={{ color: T.faint }}>
                      {WEAK_AGENT.team} · {WEAK_AGENT.id} · {WEAK_AGENT.calls} calls · {WEAK_AGENT.vio} findings this month
                    </p>
                  </div>
                  <ScoreRing value={WEAK_AGENT.score} size={58} stroke={6} color={C.red} label="score" play={detail} />
                  <Btn icon="send">Open coaching plan</Btn>
                </div>
                ...
                <div className="mt-4 flex gap-3.5">
                  <div data-shot="detail-radar" className="w-[236px] shrink-0">
                    <p className="mb-1 text-[10px] font-semibold uppercase tracking-[0.1em]" style={{ color: T.faint }}>
                      Criteria breakdown vs team average
                    </p>
                    ...
                    <Legend
                      className="mt-1"
                      items={[
                        { label: "Team average", color: C.blue },
                  ...
                  <div data-shot="notes" className="min-w-0 flex-1">
                    <div className="mb-2 flex items-center justify-between">
                      ...
                      <Badge tone="blue">generated from evidence</Badge>
                    </div>
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-[13px] font-semibold" style={{ color: T.text }}>
                      {WEAK_AGENT.repetitive}% of this agent's calls are repetitive, script-driven intents
                    </p>
                    ...
                    <p className="mt-1.5 text-[11px]" style={{ color: T.sub }}>
                      Debt inquiry, receipt verification and payment plan questions — where 11 of the 14 findings happened.
                      Complex enforcement files stay with the human.
                    </p>
                  </div>
                  <span data-click="to-automation">
                    <Btn primary={!handedOver} tone={C.purple} icon={handedOver ? "check" : "arrowRight"}>
                      {handedOver ? "Handed over to the assistant" : "Hand over to the assistant"}
                    </Btn>
                  </span>
                </div>
              </Card>
