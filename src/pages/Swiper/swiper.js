...
const SwiperPage = ({  isOutside = false, }) => {
  ...
  //Per-call pricing (TL), same tiers as the desktop pay plans.
  const planOneValue = 1.25;
  const planTwoValue = 0.95;
  const planThreeValue = 0.75;

  const plans = {
    0: { name: 'Starter', value: planOneValue },
    1: { name: 'Growth', value: planTwoValue },
    2: { name: 'Enterprise', value: planThreeValue },
  };

  const svgTikDark = (
    <svg width="14" height="14" viewBox="0 0 18 13" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path
  ...
  const choosePlan = (idx) => {
    setActiveIndex(idx);
    setGlobalState("mainPayAmount", plans[idx].value);
    setGlobalState("paymentPlan", plans[idx].name);
    setIsPopupOpen(true);
  };
  ...
  return (
    <>
      ...
      <ResponsivePopup open={isPopupOpen} onClose={onDismiss}>
        <PlanRequestSuccess planName={plans[activeIndex]?.name} planValue={plans[activeIndex]?.value} />
      </ResponsivePopup>
        ...
        >
         ...
         <header className="flex items-center justify-between">
          ...
          <svg
              ...
              <p className="text-[22px]">Starter</p>
          </div>
         ...
         <div className="flex flex-col gap-1 text-sm ">
           ...
           <div className=" flex items-center gap-3">
             {svgTikDark}
             <p class="color--black">100% of your recorded calls audited</p>
           </div>


           <div class="flex items-center gap-3">
             {svgTikDark}
             <p class="color--black">Compliance score + flagged findings</p>
           </div>

           <div class="flex items-center gap-3">
             {svgTikDark}
             <p class="color--black">Transcript, timestamp &amp; audio evidence</p>
           </div>

           <div class="flex items-center gap-3">
             {svgTikDark}
             <p class="color--black">E-mail support</p>
           </div>


         </div>

         <p onClick={()=>{ choosePlan(0) }} style={{ background: 'linear-gradient(135deg, var(--color-primary-light) 0%, var(--color-primary) 100%)', boxShadow: '0 4px 14px rgba(1,103,140,0.35), inset 0 1px 0 rgba(255,255,255,0.15)' }} className='mt-3 w-full flex py-1.5 pl-4 pr-1 rounded-full items-center justify-between gap-2 text-white text-sm'>
           <span className='flex-1 text-center font-semibold'>Choose Plan</span>
           <span className='shrink-0 rounded-full bg-white/20 h-7 w-7 flex items-center justify-center' style={{ boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.3)' }}>
          ...
          <SwiperSlide  className="flex flex-col gap-2 border  bg-slate-100 text-pri  px-3 pt-3 ">
            ...
            <div className="flex gap-3 items-center justify-center">
              ...
              <p className="text-[22px]">Growth</p>

             </div>
            <p style={{ background: 'linear-gradient(135deg, var(--color-primary-light) 0%, var(--color-primary) 100%)' }} className='px-4 py-1 rounded-full text-white'>Best Value</p>
             </div>
            ...
            <div className="flex flex-col gap-1 text-sm ">
              ...
              <div className=" flex items-center gap-3">
                {svgTikDark}
                <p class="color--black">Everything in Starter</p>
              </div>

              <div class="flex items-center gap-3">
                {svgTikDark}
                <p class="color--black">Custom rulebook configuration</p>
              </div>

              <div class="flex items-center gap-3">
              {svgTikDark}
                <p class="color--black">Voice assistant add-on, from 1 TL / min</p>
              </div>

              <div class="flex items-center gap-3">
              {svgTikDark}
                <p class="color--black relative">Priority onboarding &amp; support</p>
              </div>

            </div>

            <p onClick={()=>{ choosePlan(1) }} style={{ background: 'linear-gradient(135deg, var(--color-primary-light) 0%, var(--color-primary) 100%)', boxShadow: '0 4px 14px rgba(1,103,140,0.35), inset 0 1px 0 rgba(255,255,255,0.15)' }} className='mt-3 w-full flex py-1.5 pl-4 pr-1 rounded-full items-center justify-between gap-2 text-white text-sm'>
           <span className='flex-1 text-center font-semibold'>Choose Plan</span>
           <span className='shrink-0 rounded-full bg-white/20 h-7 w-7 flex items-center justify-center' style={{ boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.3)' }}>
             <svg width='13' height='13' viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='2.8' strokeLinecap='round' strokeLinejoin='round'><path d='M5 13l4 4L19 7'/></svg>
           </span>
           ...
           <SwiperSlide  style={{ background: 'linear-gradient(155deg, var(--color-primary) 0%, var(--color-primary-light) 55%, rgba(255,255,255,0.35) 135%)' }} className="flex  flex-col gap-2    text-white px-5 pt-5">
            <header className="flex items-center justify-between">
             <div className="flex gap-3 items-center justify-center">
             <svg
                 width="32"
               ...
               >
                 ...
                 <p className="text-[22px]">Enterprise</p>
             </div>
            ...
            <div className="flex flex-col gap-1 text-sm">
              <div className=" flex items-center gap-3">
                {svgTikWhite}
                <p class="">Everything in Growth</p>
              </div>

              <div class="flex items-center gap-3">
                {svgTikWhite}
                <p class="">On-premise / dedicated infrastructure</p>
              </div>

              <div class="flex items-center gap-3">
                {svgTikWhite}
                <p class="">Dedicated compliance advisor</p>
              </div>

              <div class="flex items-center gap-3">
                {svgTikWhite}
                <p class="">Custom SLA &amp; integrations</p>
              </div>

            </div>

            <p onClick={()=>{ choosePlan(2) }} className='mt-3 w-full flex py-1.5 pl-4 pr-1 rounded-full items-center justify-between gap-2 bg-white text-primary text-sm'>
           <span className='flex-1 text-center font-semibold'>Choose Plan</span>
           <span className='shrink-0 rounded-full bg-primary/10 h-7 w-7 flex items-center justify-center'>
             <svg width='13' height='13' viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='2.8' strokeLinecap='round' strokeLinejoin='round'><path d='M5 13l4 4L19 7'/></svg>
           </span>
         </p>
