...
const IconsMobile = () => {
  ...
  return (
    ...
    <div className="text-[#000000]  mt-10 gap-6 font-product self-center items-center justify-center flex flex-col ">
      ...
      <p className="text-[30px] w-[96vw]  self-center text-center">A hassle-free experience, centered around you.</p>
   
      <div className=" flex flex-col gap-6 self-center justify-center w-[90vw] ">
        
        {icons.map((company,idx) => (
          <div key={idx} className="flex w-[full]  gap-4  items-center">
          <img
            className="w-8"
            src={company.img}
          />
          <p className="text-center text-[18px]">{company.text}</p>
        </div>
        ))}
      </div>
    </div>
